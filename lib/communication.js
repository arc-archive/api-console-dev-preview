...
class CommunicationBridge {
  ...
  inject(mainFile) {
    ...
    .then((ast) => {
      if (!ast) {
        this.logger.error('Console main file is not a HTML document.');
        throw new Error('Not a HTML document.');
      }
      this.logger.info('HTML file parsed.');
      this.doc = ast;
  ...
  _getScriptContent() {
    ...
    return this._readInjectScript()
    .then((content) => this._injectScriptBootstrap(content))
    .then((content) => this._babelifyScript(content));
  }
  /**
   * Reads content of the `script-inject.js` file.
   * @return {Promise} Resolved promise with file content.
   */
  _readInjectScript() {
  ...
  /**
   * Adds script initialization command which setups port and host for bridge
   * connection.
   *
   * @param {String} content Communication bridge script content
   * @return {String} Script content with initialization command.
   */
  _injectScriptBootstrap(content) {
    this.logger.info('Script read. Adding connection details:');
    this.logger.info(`Host: ${this.host}, port: ${this.port}`);
    content += '\n';
    content += 'window.addEventListener(\'WebComponentsReady\',';
    content += 'function() {';
  ...
  /**
   * Uses babel to translate script for legacy browsers (IE, old safari).
   * @param {String} content Communication bridge script content
   * @return {String} Script content ready to be served with most browsers.
   */
  _babelifyScript(content) {
    this.logger.info('Translating script for Safari and IE.');
    try {
